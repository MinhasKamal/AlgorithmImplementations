...
int main(){
    ...
    input.open("InfixToPostfix_Input.txt");
    ...
    vector<string> postfix;     //postfix form of the input
	stack<string> tempOpertStore; //temporarily stores operators

	tempOpertStore.push("(");
	for(int i=0; !tempOpertStore.empty(); i++){ //conversion
        if(!inputElement[i].opert){
            postfix.push_back(inputElement[i].str);
        }
        else{
            if(inputElement[i].str[0]=='('){
                ...
                while(tempOpertStore.top()[0]!='('){
                    postfix.push_back(tempOpertStore.top());
                    tempOpertStore.pop();
                }
	...
	for(int i=0; i<postfix.size(); i++) //the output
		cout << postfix[i] << " " ;
	cout << "\n\n";
    return 0;
}
